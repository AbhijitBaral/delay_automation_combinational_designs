...
module carry_compressor #(parameter K=28)(
    input[27:0] A,B,
    input Cin,
    ...
    output [13:0]group_carry,
    output Cout
    );

//--------------------------------------------------------------
wire [13:0]group_propagate, group_generate;
//--------------------------------------------------------------
...
.CO({dummyCO[1:0] , group_carry[13:12]}),
.O ({rca_sum[1:0] , dummySO[1:0]}),
.CI(group_carry[11]),
.CYINIT(),
.DI({rca_gen[1:0] , group_generate [13:12]}),
.S ({rca_prp[1:0] , group_propagate[13:12]})
);
