...
module carry_compressor #(parameter K=30)(
    input[29:0] A,B,
    input Cin,
    ...
    output [14:0]group_carry,
    output Cout
    );

//--------------------------------------------------------------
wire [14:0]group_propagate, group_generate;
//--------------------------------------------------------------
...
.CO({dummyCO[0:0] , group_carry[14:12]}),
.O ({rca_sum[0:0] , dummySO[2:0]}),
.CI(group_carry[11]),
.CYINIT(),
.DI({rca_gen[0:0] , group_generate [14:12]}),
.S ({rca_prp[0:0] , group_propagate[14:12]})
);
