`timescale 1ns / 1ps
module RCA #(parameter M=10, K=22)(
	input [8:0]A,B,
	input Cin,
	output [8:0]sum,
	output Cout
...
.CO({dummyCO[2:0]  , carry[8:8]}),
.O ({dummySO[2:0]  , sum[8:8]}),
.CI(carry[7]),
.CYINIT(),
.DI({dummyDI[2:0], A[8:8]}),
.S ({dummyS[2:0] , Propagate[8:8]})
);

assign Cout=carry[9-1];
