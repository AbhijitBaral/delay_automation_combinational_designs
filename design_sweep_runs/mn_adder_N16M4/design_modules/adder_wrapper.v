...
`timescale 1ns / 1ps
module mn_adder_wrapper #(parameter N=16, M=4)(
    input [15:0]A,B,
    input Cin, clk, rst,
    output [15:0] sum,
    output Cout
    ...
    mn_adder #(.N(16), .M(4))adder_inst(.A({A[15:1], in_reg}), .B(B), .Cin(Cin), .sum({driver_fa, sum[14:12], driver_So, sum[10:0]}), .Cout(Cout));
