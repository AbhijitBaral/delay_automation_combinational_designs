...
module carry_compressor #(parameter K=10)(
    input[9:0] A,B,
    input Cin,
    ...
    output [4:0]group_carry,
    output Cout
    );

//--------------------------------------------------------------
wire [4:0]group_propagate, group_generate;
//--------------------------------------------------------------
...
//For K=2
(*RLOC = "X0Y1"*) CARRY4 CC_carry_chain_last(
.CO({dummyCO[2:0] , group_carry[4:4]}),
.O ({rca_sum[2:0] , dummySO[0:0]}),
.CI(group_carry[3]),
.CYINIT(),
.DI({rca_gen[2:0] , group_generate [4:4]}),
.S ({rca_prp[2:0] , group_propagate[4:4]})
);
...
(*RLOC = "X0Y1"*) LUT6 #(.INIT(64'h0000000000000006))RCA_lut_0(
.O(rca_prp[0]),
.I0(a_rem[0]),
.I1(b_rem[0]),
.I2(1'b0),
...
(*RLOC = "X0Y1"*) LUT6 #(.INIT(64'h0000000000000006))RCA_lut_1(
.O(rca_prp[1]),
.I0(a_rem[1]),
.I1(b_rem[1]),
.I2(1'b0),
...
(*RLOC = "X0Y1"*) LUT6 #(.INIT(64'h0000000000000006))RCA_lut_2(
.O(rca_prp[2]),
.I0(a_rem[2]),
.I1(b_rem[2]),
.I2(1'b0),
