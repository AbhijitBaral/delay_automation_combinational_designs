...
module carry_compressor #(parameter K=124)(
    input[123:0] A,B,
    input Cin,
    ...
    output [61:0]group_carry,
    output Cout
    );

//--------------------------------------------------------------
wire [61:0]group_propagate, group_generate;
//--------------------------------------------------------------
...
.CO({dummyCO[1:0] , group_carry[61:60]}),
.O ({rca_sum[1:0] , dummySO[1:0]}),
.CI(group_carry[59]),
.CYINIT(),
.DI({rca_gen[1:0] , group_generate [61:60]}),
.S ({rca_prp[1:0] , group_propagate[61:60]})
);
...
(*RLOC = "X0Y15"*) LUT6 #(.INIT(64'h0000000000000006))RCA_lut_1(
.O(rca_prp[1]),
.I0(a_rem[1]),
.I1(b_rem[1]),
.I2(1'b0),
