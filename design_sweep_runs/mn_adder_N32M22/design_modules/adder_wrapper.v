...
`timescale 1ns / 1ps
module mn_adder_wrapper #(parameter N=32, M=22)(
    input [31:0]A,B,
    input Cin, clk, rst,
    output [31:0] sum,
    output Cout
    ...
    mn_adder #(.N(32), .M(22))adder_inst(.A({A[31:1], in_reg}), .B(B), .Cin(Cin), .sum({driver_fa, sum[30:10], driver_So, sum[8:0]}), .Cout(Cout));
