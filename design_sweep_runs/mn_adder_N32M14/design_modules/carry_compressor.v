...
module carry_compressor #(parameter K=18)(
    input[17:0] A,B,
    input Cin,
    ...
    output [8:0]group_carry,
    output Cout
    );

//--------------------------------------------------------------
wire [8:0]group_propagate, group_generate;
//--------------------------------------------------------------
...
.CO({dummyCO[2:0] , group_carry[8:8]}),
.O ({rca_sum[2:0] , dummySO[0:0]}),
.CI(group_carry[7]),
.CYINIT(),
.DI({rca_gen[2:0] , group_generate [8:8]}),
.S ({rca_prp[2:0] , group_propagate[8:8]})
);
...
(*RLOC = "X0Y2"*) LUT6 #(.INIT(64'h0000000000000006))RCA_lut_2(
.O(rca_prp[2]),
.I0(a_rem[2]),
.I1(b_rem[2]),
.I2(1'b0),
