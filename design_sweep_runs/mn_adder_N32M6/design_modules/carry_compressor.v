...
module carry_compressor #(parameter K=26)(
    input[25:0] A,B,
    input Cin,
    //     a[K+3-((K/2)%4):K],   b[K+3-((K/2)%4):K]
    input  [2:0]a_rem, b_rem,         //operand bits belonging to the last M bits
    output [2:0]rca_sum,              //Sum of the [K+3-((K/2)%4:K)] stages of addition
    output [12:0]group_carry,
    output Cout
    );

//--------------------------------------------------------------
wire [12:0]group_propagate, group_generate;
//--------------------------------------------------------------
...
//For K=2
(*RLOC = "X0Y3"*) CARRY4 CC_carry_chain_last(
.CO({dummyCO[2:0] , group_carry[12:12]}),
.O ({rca_sum[2:0] , dummySO[0:0]}),
.CI(group_carry[11]),
.CYINIT(),
.DI({rca_gen[2:0] , group_generate [12:12]}),
.S ({rca_prp[2:0] , group_propagate[12:12]})
);
...
wire [2:0]rca_prp, rca_gen;
assign rca_gen = a_rem;

(*RLOC = "X0Y3"*) LUT6 #(.INIT(64'h0000000000000006))RCA_lut_0(
.O(rca_prp[0]),
.I0(a_rem[0]),
.I1(b_rem[0]),
.I2(1'b0),
...
(*RLOC = "X0Y3"*) LUT6 #(.INIT(64'h0000000000000006))RCA_lut_1(
.O(rca_prp[1]),
.I0(a_rem[1]),
.I1(b_rem[1]),
.I2(1'b0),
...
(*RLOC = "X0Y3"*) LUT6 #(.INIT(64'h0000000000000006))RCA_lut_2(
.O(rca_prp[2]),
.I0(a_rem[2]),
.I1(b_rem[2]),
.I2(1'b0),
