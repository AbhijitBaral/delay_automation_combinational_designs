...
module carry_compressor #(parameter K=14)(
    input[13:0] A,B,
    input Cin,
    ...
    output [6:0]group_carry,
    output Cout
    );

//--------------------------------------------------------------
wire [6:0]group_propagate, group_generate;
//--------------------------------------------------------------
...
.CO({dummyCO[0:0] , group_carry[6:4]}),
.O ({rca_sum[0:0] , dummySO[2:0]}),
.CI(group_carry[3]),
.CYINIT(),
.DI({rca_gen[0:0] , group_generate [6:4]}),
.S ({rca_prp[0:0] , group_propagate[6:4]})
);
